...
return <div className="products" id={props.element.asin} data-category = {props.element.catergory}>
        <div className="image_container "><img className="image" width={450} height={245} src={props.element.image} alt=''/></div>
        <div className="product_title">{props.element.title}</div>
        ...
        <span className={'favourite '+ (favSel.includes(props.element.asin) ? 'bi-heart-fill' : 'bi-heart')} onClick={()=>{favourite(props.element.asin)}}></span></div>
     </div>;
}